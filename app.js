...
(function(){
  ...
  class MarkerWrapper {
    constructor(latlng, options){
      ...
      this._gl = new maplibregl.Marker({ element: this._el, anchor: "top-left" })
        .setLngLat([lng, lat]);

      // Popup nyitás marker kattintásra (Leaflet-szerű)
  ...
  this._el.appendChild(inner);
    ...
    remove(){
      try { if (this._popup) this._popup.remove(); } catch(_){}
    ...
    setLatLng(latlng){
      ...
      try {
        if (this._popup && this._popup.isOpen()) this._popup.setLngLat([lng, lat]);
      } catch(_){}
      return this;
    }
    ...
    openPopup(){
      ...
      // Leaflet "popupopen" event emuláció
      const ev = {
        popup: { getElement: () => this._popup.getElement() }
      };
      for (const fn of this._popupOpenHandlers) {
        try { fn(ev); } catch(e){ console.warn("popupopen handler error", e); }
      }
      return this;
    }
    ...
    on(evt, fn){
      if (evt === "popupopen" && typeof fn === "function") {
        this._popupOpenHandlers.push(fn);
      }
      return this;
    }
  }
